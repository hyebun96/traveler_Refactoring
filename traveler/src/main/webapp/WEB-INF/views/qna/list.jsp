...
<div class="qna">
                ...
                <td class="qna-title">
                    <c:choose>
                        <c:when test="${dto.depth==0}">
                            ...
                            <c:if test="${sessionScope.member.userId!=dto.userId && sessionScope.member.userId!='admin'}"><a
                                    onclick="memberOnly();">${dto.subject}</a></c:if>
                        </c:when>
                        <c:otherwise>
                            &nbsp;└&nbsp;re:
                            ...
                            <c:if test="${sessionScope.member.userId!=qnaId && sessionScope.member.userId!='admin'}"><a
                                    onclick="memberOnly();">${dto.subject}</a></c:if>
                        </c:otherwise>
                    </c:choose>
                </td>
    ...
    <div>
        ...
        <button type="button" onclick="javascript:location.href='<%=cp%>/qna/write.do';" style="float: right">글등록
        </button>
    </div>
